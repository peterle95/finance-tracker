...
| Change Electron process/API wiring | [Architecture](architecture/overview.md#electron-process-boundary) | `modern-desktop/src/main/index.ts`, `src/preload/index.ts`, `src/shared/types.ts` | `FinanceApi`, `ipcMain.handle`, `contextBridge.exposeInMainWorld` | `src/renderer/App.test.tsx` and affected renderer/API tests | `npm run typecheck` (from `modern-desktop`) |
| Change Electron persistence | [DataStore](modern-desktop/data-store.md) | `modern-desktop/src/main/data-store.ts`, `src/main/file-utils.ts` | `DataStore.load`, `saveDocument`, `saveDocumentNow`, `saveQueue` | `modern-desktop/src/main/data-store.test.ts`, `file-utils.test.ts` | `npm test -- src/main/data-store.test.ts` (from `modern-desktop`) |
| Change Electron formulas/reports | [Domain](modern-desktop/domain.md) | `modern-desktop/src/shared/finance.ts`, settings modules | `FinanceDocument`, normalization and shared finance functions | `src/shared/finance.test.ts`, settings suites | `npm test -- src/shared/finance.test.ts` (from `modern-desktop`) |
| Change Electron keyboard accessibility or renderer shortcuts | [React UI](modern-desktop/ui.md#keyboard-navigation) | `modern-desktop/src/renderer/keyboard-navigation.ts`, `src/renderer/App.tsx`, `src/renderer/components/SettingsScreen.tsx` | `useKeyboardNavigation`, `normalizeKeyboardNavigationSettings`, `KeyboardNavigationSettings` | `src/renderer/keyboard-navigation.test.tsx`, `src/renderer/App.test.tsx`, `src/renderer/components/SettingsScreen.test.tsx` | `npm test -- src/renderer/keyboard-navigation.test.tsx src/renderer/App.test.tsx src/renderer/components/SettingsScreen.test.tsx` (from `modern-desktop`) |
